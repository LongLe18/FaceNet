    ...
    public partial class User_Interface : Form
    {
        ...
        private void BtnStart_Click_1(object sender, EventArgs e)
        {
            while (true)
            {
                if (cam.IsRunning == false)
                    cam.Start();
                if (txtName.Text == "" || txtID.Text == "" || txtAge.Text == "")    //Name or Id is empty
                {
                ...
                else
                {
                    ...
                    if (click > 22)                 //take more than 5 pics, break
                    {
                        MessageBox.Show("Bạn đã chụp đủ ảnh !!!");
                        break;
                    }
                    else
                    {
                        ...
                        string server_path = "http://" + server_ip + ":8000/capture?name=" + txtName.Text + "&id=" + txtID.Text + "&age=" + txtAge.Text + "&pic=" + click.ToString();
        ...
        private void Timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (items[0] != "")
            {
                ...
                newGraphics.DrawString("Name: " + val[0].Split('_')[1], new Font("Tahoma", 25), Brushes.Yellow, int.Parse(val[1]), int.Parse(val[2]) - 50);
                newGraphics.DrawString("ID: " + val[0].Split('_')[0] + " - Age: " + val[5], new Font("Tahoma", 25), Brushes.Yellow, int.Parse(val[1]), int.Parse(val[2]) - 100);
